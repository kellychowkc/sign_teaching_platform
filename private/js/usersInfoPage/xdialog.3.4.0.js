...
window.xdialog = (function () {
  ...
  function createDialog(options, overlayElement) {
    ...
    dialogElement.setAttribute(
      "class",
      "xd-dialog xd-center " + effect.clazz + " " + options.extraClass
    );
    ...
    if (options.body) {
      if (utils.isString(options.body)) {
        innerHTML +=
          '<div class="xd-body"><div class="xd-body-inner">' + options.body + "</div></div>";
      } else {
        let srcElement = null;
    ...
    if (options.beforecreate) {
      if (
        options.beforecreate(callbackParam(dialogElement, null, overlayElement, null)) === false
      ) {
        return null;
      }
    }
    ...
    options.aftercreate &&
      options.aftercreate(callbackParam(dialogElement, null, overlayElement, null));
    ...
    Object.keys(buttonInfos).forEach(function (name) {
      if (buttonInfos[name].html) {
        ...
        html +=
          ...
          style +
          '" class="' +
          buttonInfos[name].clazz +
          '">' +
          buttonInfos[name].text +
          "</button>";
      }
    });
  ...
  function create(options) {
      ...
      function checkStatusAndShow() {
        if (preparedForShow) {
          if (options.beforeshow) {
            if (
              options.beforeshow(callbackParam(dialogElement, dialog, overlayElement, null)) ===
              false
            ) {
              return;
            }
          ...
          showMe();
          options.aftershow &&
            options.aftershow(callbackParam(dialogElement, dialog, overlayElement, null));
        } else {
    ...
    function hide() {
      if (options.beforehide) {
        if (
          options.beforehide(callbackParam(dialogElement, dialog, overlayElement, null)) === false
        ) {
          return false;
        }
      }
      ...
      options.afterhide &&
        options.afterhide(callbackParam(dialogElement, dialog, overlayElement, null));
    }
    ...
    function doOk(e) {
      if (
        options.onok &&
        options.onok(callbackParam(dialogElement, dialog, overlayElement, e)) === false
      ) {
        return;
      }
    ...
    function doCancel(e) {
      if (
        options.oncancel &&
        options.oncancel(callbackParam(dialogElement, dialog, overlayElement, e)) === false
      ) {
        return;
      }
    ...
    function doDelete(e) {
      if (
        options.ondelete &&
        options.ondelete(callbackParam(dialogElement, dialog, overlayElement, e)) === false
      ) {
        return;
      }
    ...
    function destroy() {
      if (
        options.ondestroy &&
        options.ondestroy(callbackParam(dialogElement, dialog, overlayElement, null)) === false
      ) {
        return;
      }
  ...
  function dragElement(ondrag, destElement, srcElement, onclick) {
    ...
    function closeDragElement(e) {
      ...
      if (
        Math.abs(e.clientX - mouseDownEvent.clientX) +
          Math.abs(e.clientY - mouseDownEvent.clientY) <
          dragAsClick.distance &&
        e.timeStamp - mouseDownEvent.timeStamp < dragAsClick.timeout
      ) {
        onclick && onclick(e);
      }
