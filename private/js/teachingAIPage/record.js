...
export const setupCamera = () => {
  ...
  const setupCamera = async () => {
    ...
    if (videoOutput !== null) {
      const mediaStream = await navigator.mediaDevices.getUserMedia(constraints).catch((err) => {
...
export const captureImage = () => {
  ...
  return {
    imageData: canvasCtx?.getImageData(0, 0, canvasElem.width, canvasElem.height),
