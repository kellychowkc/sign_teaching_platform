...
export class StudentController {
  ...
  editStudentInfo = async (req: Request, res: Response) => {
    try {
      ...
      const editInfoResult = await this.studentService.editStudentInfo(
        userId,
        fname,
        lname,
        email,
  ...
  insertBookingLesson = async (req: Request, res: Response) => {
    try {
      ...
      const bookedResult = await this.studentService.insertBookingData(
        userId,
        teacherId,
        bookingDateTime
      );
    ...
    try {
      ...
      if (orderData) {
        ...
        if (lessonDataInOrder.length === 0) {
          ...
          res.status(200).json({
            success: true,
            message: { order: thisOrderData, lessonList: lessonDataInOrder },
          });
        }
      }
    } catch (err) {
  ...
  displayLessonForStudent = async (req: Request, res: Response) => {
    try {
      ...
      const lessonData = await this.studentService.getLessonForStudent(
        userId,
        upLimitDay,
        downLimitDay
      );
