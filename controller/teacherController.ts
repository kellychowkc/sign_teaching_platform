...
export class TeacherController {
  ...
  editTeacherInfo = async (req: Request, res: Response) => {
    teacherImage.parse(req, async (err, fields, files) => {
      try {
        ...
        const editInfoResult = await this.teacherService.editTeacherInfo(
          userId,
          fname,
          lname,
          email,
        ...
        if (files["image"]) {
          ...
          if (oldImage) {
            if (oldImage !== "teacher_icon.png") {
              fs.unlink(
                path.join(__dirname, `../private/assets/usersImages/${oldImage}`),
                (err) => {
                  if (err) {
                    logger.error(err.toString());
                    throw err;
  ...
  displayLessonForTeacher = async (req: Request, res: Response) => {
    try {
      ...
      const lessonData = await this.teacherService.getLessonForTeacher(
        userId,
        upLimitDay,
        downLimitDay
      );
