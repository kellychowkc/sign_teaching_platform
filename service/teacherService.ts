...
export class TeacherService {
  ...
  async editTeacherInfo(
    ...
    try {
      await txn("users")
        ...
        .where("id", id);
      await txn("teachers").update({ teacher_description: description }).where("user_id", id);
      await txn.commit();
      return true;
    } catch (err) {
      await txn.rollback();
  ...
  async editTeacherImage(id: number, image: string) {
    const teacherOldData: Array<{ teacher_image: string }> = await this.knex("teachers")
  ...
  async getCanTeachingTime(id: number) {
    const teacherBookingData: Array<{ weekday: string; booking_time: string }> = await this.knex(
      "can_booking_table"
    )
  ...
  async editCanTeachingTime(id: number, weekdayData: string, timeData: string) {
    const txn = await this.knex.transaction();
    try {
      const teacherData: Array<{ id: number }> = await txn("teachers")
        .select("id")
        .where("user_id", id);

      // check have already data in "time_table"
      const matchTime: Array<{ id: number }> = await txn("time_table")
        .select("id")
      ...
      if (matchTime.length === 0) {
        ...
        const newTimeData: Array<{ id: number }> = await txn("time_table")
          .insert({
            weekday: weekdayData,
            booking_time: timeData,
          })
          .returning("id");
        await txn("can_booking_table").insert({
        ...
        const hasData: Array<{ id: number }> = await txn("can_booking_table")
          .select("id")
  ...
  async insertLessonLink(id: number, link: string) {
    const matchLink: Array<{ id: number }> = await this.knex("lessons")
      .select("id")
