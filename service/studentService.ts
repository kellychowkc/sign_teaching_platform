...
export class StudentService {
  ...
  async getStudentInfo(id: number) {
    const result: Array<{
      ...
      .select("username", "first_name", "last_name", "email", "phone_num")
      .where("id", id);
    return result[0];
  }
  ...
  async editStudentPass(id: number, oldPass: string, newPass: string) {
    const checkPass: Array<{ password: string }> = await this.knex("users")
      .select("password")
      .where("id", id);
  ...
  async selectBookedDate(id: number) {
    const bookedDate: Array<{ date_time: Date }> = await this.knex("lessons")
      .select("date_time")
      .where("teacher_id", id);
    return bookedDate;
  }
  ...
  async insertBookingData(userId: number, teacherId: number, dateTime: string) {
    ...
    try {
      const orderDataOfNowUse: Array<{ id: number; remaining_lesson_num: number }> = await txn(
        "orders"
      )
        .select("id", "remaining_lesson_num")
        .where("user_id", userId)
        .andWhere("remaining_lesson_num", ">", 0)
        .orderBy("created_at");
  ...
  async getPackagesRecord(id: number) {
    const priceRecord: Array<{ package_id: number }> = await this.knex("packages_prices").select(
      "package_id"
    );
    let packagesRecord = [];
  ...
  async getAllOrderData(id: number) {
    const ordersRecord: Array<{
      id: number;
      package_name: string;
      total_lesson_num: number;
      remaining_lesson_num: number;
      created_at: Date;
    ...
    if (ordersRecord.length > 0) {
      for (let order of ordersRecord) {
        const lessonStatus: Array<{ status: string }> = await this.knex("lessons")
          .select("status")
  ...
  async getPackagePrice(id: number) {
    const packageData: Array<{ price: number }> = await this.knex("packages_prices")
      .select("price")
      .where("package_id", id)
  ...
  async insertNewOrder(userId: number, packageId: number) {
    const packageData: Array<{ total_lesson_num: number }> = await this.knex("packages")
      .select("total_lesson_num")
      .where("id", packageId);
    return await this.knex("orders").insert({
